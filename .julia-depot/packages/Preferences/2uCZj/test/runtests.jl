...
function activate_and_run(project_dir::String, code::String; env::Dict = Dict())
    mktempdir() do dir
        ...
        out = Pipe()
        cmd = setenv(
            `$(Base.julia_cmd()) --project=$(project_dir) $(dir)/test_code.jl`,
            env...,
            "JULIA_DEPOT_PATH" => Base.DEPOT_PATH[1],
        )
...
up_uuid =
    UUID(TOML.parsefile(joinpath(@__DIR__, "UsesPreferences", "Project.toml"))["uuid"])
...
isdefined(Base, :isprecompiled) && @test !Base.isprecompiled(Base.identify_package("UsesPreferences"))
""" * cuda_test_base
        output = activate_and_run(
            up_path,
            cuda_test_did_precompile;
            env = Dict("JULIA_DEBUG" => "loading"),
        )
...
""",
        ...
        """
        output =
            activate_and_run(up_path, username_test; env = Dict("JULIA_DEBUG" => "loading"))
        @test !did_precompile(output)
            ...
            mktempdir() do dir
                activate(dir) do
                    ...
                    try
                        ...
                        @test_throws ArgumentError set_preferences!(
                            "UsesPreferences",
                            "location" => "exists",
                        )
                        @test_throws ArgumentError has_preference(
                            "UsesPreferences",
                            "location",
                        )
                        @test_throws ArgumentError load_preference(
                            "UsesPreferences",
                            "location",
                        )
                        @test_throws ArgumentError delete_preferences!(
                            "UsesPreferences",
                            "location",
                        )
        ...
        mktempdir() do env_dir
            try
                ...
                open(joinpath(env_dir, "Project.toml"), "w") do io
                    TOML.print(
                        io,
                        Dict("deps" => Dict("UsesPreferences" => string(up_uuid))),
                    )
                end
                ...
                activate(env_dir) do
                    ...
                    @test proj["preferences"]["UsesPreferences"]["location"] ==
                          "outer_public"
                ...
                activate(env_dir) do
                    set_preferences!(
                        up_uuid,
                        "nested" =>
                            Dict("nested2" => Dict("a" => 1, "b" => 2), "leaf" => "hello");
                        export_prefs = true,
                    )
                    set_preferences!(up_uuid, "nested" => Dict("nested2" => Dict("b" => 3)))
                ...
                activate(up_path) do
                    set_preferences!(
                        up_uuid,
                        "nested" =>
                            Dict("nested2" => Dict("a" => "foo"), "leaf" => "world"),
                    )
                    nested = load_preference(up_uuid, "nested")
                    @test isa(nested, Dict) && haskey(nested, "nested2")
                ...
                activate(env_dir) do
                    ...
                    set_preferences!(
                        up_uuid,
                        "nested" => Dict("nested2" => missing, "leaf" => nothing);
                        force = true,
                    )
                    nested = load_preference(up_uuid, "nested")
                    @test isa(nested, Dict) && haskey(nested, "nested2")
                ...
                mktempdir() do empty_project_dir
                    ...
                    activate(empty_project_dir) do
                        ...
                        set_preferences!(
                            up_uuid,
                            "location" => "overridden_outer_local";
                            active_project_only = false,
                            force = true,
                        )
                        prefs = Base.parsed_toml(joinpath(env_dir, "LocalPreferences.toml"))
                        @test prefs["UsesPreferences"]["location"] ==
                              "overridden_outer_local"

                        # This will set it in the currently active project, and add `UsesPreferences`
                        # as a dependency under the `"extras"` section
                        set_preferences!(
                            up_uuid,
                            "location" => "empty_inner_local";
                            active_project_only = true,
                        )
                        prefs = Base.parsed_toml(
                            joinpath(empty_project_dir, "LocalPreferences.toml"),
                        )
                        ...
                        set_preferences!(
                            up_uuid,
                            "location" => "still_empty_inner_local";
                            active_project_only = false,
                            force = true,
                        )
                        prefs = Base.parsed_toml(
...
@testset "Pkg.test()" begin
    ...
    @test project_hash ==
          open(io -> SHA.sha256(io), joinpath(@__DIR__, "PTest", "Project.toml"))
end
...
@testset "Configuration override for non-package module" begin
    test_with_PkgAB() do
        ...
        try
            ...
            @test 1 == Base.invokelatest(
                Base.invokelatest(getproperty, PkgA, :PkgARuntimeConfig_macro),
            )
            @test 1 == Base.invokelatest(
                Base.invokelatest(getproperty, PkgA, :PkgARuntimeConfig_func),
            )
        finally
        ...
        try
            ...
            @test 2 == Base.invokelatest(
                Base.invokelatest(getproperty, PkgB, :PkgBRuntimeConfig_macro),
            )
            @test 2 == Base.invokelatest(
                Base.invokelatest(getproperty, PkgB, :PkgBRuntimeConfig_func),
            )
