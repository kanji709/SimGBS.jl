...
let kind_int_type = :UInt16
    ...
    @eval begin
        ...
        Base.convert(::Type{String}, k::Kind) =
            _kind_names[1+reinterpret($kind_int_type, k)]
        ...
        Base.:<(x::Kind, y::Kind) =
            reinterpret($kind_int_type, x) < reinterpret($kind_int_type, y)
...
#-------------------------------------------------------------------------------
# Predicates
is_contextual_keyword(k::Kind) =
    K"BEGIN_CONTEXTUAL_KEYWORDS" <= k <= K"END_CONTEXTUAL_KEYWORDS"
is_error(k::Kind) =
    K"BEGIN_ERRORS" <= k <= K"END_ERRORS" || k == K"ErrorInvalidOperator" || k == K"Error**"
is_keyword(k::Kind) = K"BEGIN_KEYWORDS" <= k <= K"END_KEYWORDS"
is_block_continuation_keyword(k::Kind) =
    K"BEGIN_BLOCK_CONTINUATION_KEYWORDS" <= k <= K"END_BLOCK_CONTINUATION_KEYWORDS"
