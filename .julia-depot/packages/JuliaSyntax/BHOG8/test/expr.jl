@testset "Expr parsing with $method" for method in ["build_tree", "SyntaxNode conversion"]
    parseatom, parsestmt, parseall = if method == "build_tree"
        (
        ...
        (
            (s; kws...) ->
                Expr(JuliaSyntax.parseatom(SyntaxNode, s; keep_parens = true, kws...)),
            (s; kws...) ->
                Expr(JuliaSyntax.parsestmt(SyntaxNode, s; keep_parens = true, kws...)),
            (s; kws...) ->
                Expr(JuliaSyntax.parseall(SyntaxNode, s; keep_parens = true, kws...)),
        )
    end
    ...
    @testset "Line numbers" begin
        @testset "Blocks" begin
            ...
            @test parsestmt("(a;b;c)") ==
                  Expr(:block, :a, LineNumberNode(1), :b, LineNumberNode(1), :c)
            @test parsestmt("begin end") == Expr(:block, LineNumberNode(1))
            @test parsestmt("(;;)") == Expr(:block)

            @test parseall("a\n\nb") ==
                  Expr(:toplevel, LineNumberNode(1), :a, LineNumberNode(3), :b)
            @test parsestmt("a;b") == Expr(:toplevel, :a, :b)

            @test parsestmt("module A\n\nbody\nend") == Expr(
                :module,
                true,
                :A,
                Expr(:block, LineNumberNode(1), LineNumberNode(3), :body),
            )
        end
        ...
        @testset "Function definition lines" begin
            ...
            @test parsestmt("f() = 1") ==
                  Expr(:(=), Expr(:call, :f), Expr(:block, LineNumberNode(1), 1))
            @test parsestmt("macro f()\na\nend") == Expr(
                :macro,
                Expr(:call, :f),
                Expr(:block, LineNumberNode(1), LineNumberNode(2), :a),
            )

            # function/macro without methods
            @test parsestmt("function f end") == Expr(:function, :f)
            @test parsestmt("macro f end") == Expr(:macro, :f)

            # weird cases with extra parens
            @test parsestmt("function (f() where T) end") == Expr(
                :function,
                Expr(:where, Expr(:call, :f), :T),
                Expr(:block, LineNumberNode(1), LineNumberNode(1)),
            )
            @test parsestmt("function (f()::S) end") == Expr(
                :function,
                Expr(:(::), Expr(:call, :f), :S),
                Expr(:block, LineNumberNode(1), LineNumberNode(1)),
            )
        ...
        @testset "->" begin
            @test parsestmt("a -> b") == Expr(:->, :a, Expr(:block, LineNumberNode(1), :b))
            # @test parsestmt("a -> (\nb;c)") ==
        ...
        @testset "No line numbers in let bindings" begin
            @test parsestmt("let i=is, j=js\nbody\nend") == Expr(
                :let,
                Expr(:block, Expr(:(=), :i, :is), Expr(:(=), :j, :js)),
                Expr(:block, LineNumberNode(2), :body),
            )
        end
        ...
        @testset "Loops" begin
            @test parsestmt("for x=xs\n\nend") == Expr(
                :for,
                Expr(:(=), :x, :xs),
                Expr(:block, LineNumberNode(1), LineNumberNode(3)),
            )
            @test parsestmt("for x=xs\ny\nend") == Expr(
                :for,
                Expr(:(=), :x, :xs),
                Expr(:block, LineNumberNode(2), :y, LineNumberNode(3)),
            )
            @test parsestmt("while cond\n\nend") ==
                  Expr(:while, :cond, Expr(:block, LineNumberNode(1), LineNumberNode(3)))
            @test parsestmt("while cond\ny\nend") == Expr(
                :while,
                :cond,
                Expr(:block, LineNumberNode(2), :y, LineNumberNode(3)),
    ...
    @testset "Short form function line numbers" begin
        ...
        @test parsestmt("f() = xs") ==
              Expr(:(=), Expr(:call, :f), Expr(:block, LineNumberNode(1), :xs))

        @test parsestmt("f() =\n(a;b)") == Expr(
            :(=),
            Expr(:call, :f),
            Expr(:block, LineNumberNode(1), :a, LineNumberNode(2), :b),
        )

        @test parsestmt("f() =\nbegin\na\nb\nend") == Expr(
            :(=),
            Expr(:call, :f),
            Expr(:block, LineNumberNode(1), LineNumberNode(3), :a, LineNumberNode(4), :b),
        )
        ...
        @test parsestmt("let f(x) =\ng(x)=1\nend") == Expr(
            ...
            Expr(
                ...
                Expr(
                    :block,
                    LineNumberNode(1),
                    Expr(:(=), Expr(:call, :g, :x), Expr(:block, LineNumberNode(2), 1)),
                ),
            ),
            Expr(:block, LineNumberNode(3)),
        )

        # `.=` doesn't introduce short form functions
        @test parsestmt("f() .= xs") == Expr(:(.=), Expr(:call, :f), :xs)
    end

    @testset "for" begin
        @test parsestmt("for i=is body end") == Expr(
            :for,
            Expr(:(=), :i, :is),
            Expr(:block, LineNumberNode(1), :body, LineNumberNode(1)),
        )
        @test parsestmt("for i=is, j=js\nbody\nend") == Expr(
            :for,
            Expr(:block, Expr(:(=), :i, :is), Expr(:(=), :j, :js)),
            Expr(:block, LineNumberNode(2), :body, LineNumberNode(3)),
        )
    end
    ...
    @testset "Long form anonymous functions" begin
        @test parsestmt("function (xs...)\nbody end") == Expr(
            :function,
            Expr(:..., :xs),
            Expr(:block, LineNumberNode(1), LineNumberNode(2), :body),
        )
    end
    ...
    @testset "String conversions" begin
        ...
        @test parsestmt("\"\$(\"str\")\"") == Expr(:string, Expr(:string, "str"))
        # Concatenation of string chunks in triple quoted cases
        @test parsestmt("```\n  a\n  b```") ==
              Expr(:macrocall, GlobalRef(Core, Symbol("@cmd")), LineNumberNode(1), "a\nb")
        @test parsestmt("\"\"\"\n  a\n  \$x\n  b\n  c\"\"\"") ==
              Expr(:string, "a\n", :x, "\nb\nc")
        # Incomplete cases
        @test parsestmt("`x", ignore_errors = true) == Expr(
            :macrocall,
            GlobalRef(Core, Symbol("@cmd")),
            LineNumberNode(1),
            Expr(:string, "x", Expr(:error)),
        )
        @test parsestmt("`", ignore_errors = true) == Expr(
            :macrocall,
            GlobalRef(Core, Symbol("@cmd")),
            LineNumberNode(1),
    ...
    @testset "do block conversion" begin
        @test parsestmt("f(x) do y\n body end") == Expr(
            :do,
            Expr(:call, :f, :x),
            Expr(:->, Expr(:tuple, :y), Expr(:block, LineNumberNode(2), :body)),
        )
    end

    @testset "= to Expr(:kw) conversion" begin
        # Call
        @test parsestmt("f(a=1)") == Expr(:call, :f, Expr(:kw, :a, 1))
        @test parsestmt("f(; b=2)") == Expr(:call, :f, Expr(:parameters, Expr(:kw, :b, 2)))
        ...
        @test parsestmt("+(a=1,)") == Expr(:call, :+, Expr(:kw, :a, 1))
        @test parsestmt("(a=1)()") == Expr(:call, Expr(:(=), :a, 1))

        # Operator calls:  = is not :kw
        @test parsestmt("(x=1) != 2") == Expr(:call, :!=, Expr(:(=), :x, 1), 2)
        @test parsestmt("+(a=1)") == Expr(:call, :+, Expr(:(=), :a, 1))
        @test parsestmt("(a=1)'") == Expr(Symbol("'"), Expr(:(=), :a, 1))
        @test parsestmt("(a=1)'ᵀ") == Expr(:call, Symbol("'ᵀ"), Expr(:(=), :a, 1))

        # Dotcall
        @test parsestmt("f.(a=1; b=2)") == Expr(
            :.,
            :f,
            Expr(:tuple, Expr(:parameters, Expr(:kw, :b, 2)), Expr(:kw, :a, 1)),
        )

        # Named tuples
        @test parsestmt("(a=1,)") == Expr(:tuple, Expr(:(=), :a, 1))
        @test parsestmt("(a=1,; b=2)") ==
              Expr(:tuple, Expr(:parameters, Expr(:kw, :b, 2)), Expr(:(=), :a, 1))
        @test parsestmt("(a=1,; b=2; c=3)") == Expr(
            :tuple,
            Expr(:parameters, Expr(:parameters, Expr(:kw, :c, 3)), Expr(:kw, :b, 2)),
            Expr(:(=), :a, 1),
        )

        # ref
        @test parsestmt("x[i=j]") == Expr(:ref, :x, Expr(:kw, :i, :j))
        @test parsestmt("(i=j)[x]") == Expr(:ref, Expr(:(=), :i, :j), :x)
        @test parsestmt("x[a, b; i=j]") ==
              Expr(:ref, :x, Expr(:parameters, Expr(:(=), :i, :j)), :a, :b)
        # curly
        @test parsestmt("(i=j){x}") == Expr(:curly, Expr(:(=), :i, :j), :x)
        @test parsestmt("x{a, b; i=j}") ==
              Expr(:curly, :x, Expr(:parameters, Expr(:(=), :i, :j)), :a, :b)

        # vect
        @test parsestmt("[a=1,; b=2]") ==
              Expr(:vect, Expr(:parameters, Expr(:(=), :b, 2)), Expr(:(=), :a, 1))
        # braces
        @test parsestmt("{a=1,; b=2}") ==
              Expr(:braces, Expr(:parameters, Expr(:(=), :b, 2)), Expr(:(=), :a, 1))

        # dotted = is not :kw
        @test parsestmt("f(a .= 1)") == Expr(:call, :f, Expr(:.=, :a, 1))
    ...
    @testset "dotcall / dotted operators" begin
        ...
        @test parsestmt("a .< b .< c") ==
              Expr(:comparison, :a, Symbol(".<"), :b, Symbol(".<"), :c)
        @test parsestmt("a .< (.<) .< c") ==
              Expr(:comparison, :a, Symbol(".<"), Expr(:., :<), Symbol(".<"), :c)
        ...
        @test parsestmt("quote .+ end") ==
              Expr(:quote, Expr(:block, LineNumberNode(1), Expr(:., :+)))
    ...
    @testset "let" begin
        ...
        @test parsestmt("let x ; end") == Expr(:let, :x, Expr(:block, LineNumberNode(1)))
        @test parsestmt("let x::1 ; end") ==
              Expr(:let, Expr(:(::), :x, 1), Expr(:block, LineNumberNode(1)))
        @test parsestmt("let x=1,y=2 end") == Expr(
            :let,
    ...
    @testset "where" begin
        ...
        @test parsestmt("A where {X, Y; Z}") ==
              Expr(:where, :A, Expr(:parameters, :Z), :X, :Y)
    end
    ...
    @testset "macrocall" begin
        ...
        # parameters
        @test parsestmt("@m(x; a)") ==
              Expr(:macrocall, Symbol("@m"), LineNumberNode(1), Expr(:parameters, :a), :x)
        @test parsestmt("@m(a=1; b=2)") == Expr(
            :macrocall,
            Symbol("@m"),
            LineNumberNode(1),
        ...
        @test parsestmt("using A: @.") ==
              Expr(:using, Expr(Symbol(":"), Expr(:., :A), Expr(:., Symbol("@__dot__"))))

        # var""
        @test parsestmt("@var\"#\" a") ==
              Expr(:macrocall, Symbol("@#"), LineNumberNode(1), :a)
        @test parsestmt("A.@var\"#\" a") ==
              Expr(:macrocall, Expr(:., :A, QuoteNode(Symbol("@#"))), LineNumberNode(1), :a)
    ...
    @testset "generators" begin
        @test parsestmt("(x for a in as for b in bs)") == Expr(
            :flatten,
            Expr(
                :generator,
        ...
        @test parsestmt("(x for a in as, b in bs if z)") == Expr(
            :generator,
            :x,
            Expr(:filter, :z, Expr(:(=), :a, :as), Expr(:(=), :b, :bs)),
        )
        @test parsestmt("(x for a in as, b in bs for c in cs, d in ds)") == Expr(
            :flatten,
            Expr(
                :generator,
        ...
        @test parsestmt("(x for a in as for b in bs if z)") == Expr(
            :flatten,
            Expr(
                :generator,
        ...
        @test parsestmt("(x for a in as if z for b in bs)") == Expr(
            :flatten,
            Expr(
                :generator,
                Expr(:generator, :x, Expr(:(=), :b, :bs)),
        ...
        @test parsestmt("[x for a = as for b = bs if cond1 for c = cs if cond2]") == Expr(
            :comprehension,
            Expr(
                :flatten,
                Expr(
                    ...
                    Expr(
                        ...
                        Expr(
                            ...
                            Expr(
                                :generator,
                                :x,
                                Expr(:filter, :cond2, Expr(:(=), :c, :cs)),
                            ),
                            Expr(:filter, :cond1, Expr(:(=), :b, :bs)),
                        ),
                    ),
                    Expr(:(=), :a, :as),
                ),
        ...
        @test parsestmt("[x for a = as if begin cond2 end]") == Expr(
            :comprehension,
            Expr(
                :generator,
                :x,
                Expr(:filter, Expr(:block, LineNumberNode(1), :cond2), Expr(:(=), :a, :as)),
            ),
        )
    ...
    @testset "try" begin
        @test parsestmt("try x catch e; y end") == Expr(
            :try,
            Expr(:block, LineNumberNode(1), :x),
            :e,
            Expr(:block, LineNumberNode(1), :y),
        )
        @test parsestmt("try x finally y end") == Expr(
            :try,
            Expr(:block, LineNumberNode(1), :x),
            false,
            false,
        ...
        @test parsestmt("try x catch e; y finally z end") == Expr(
            :try,
            Expr(:block, LineNumberNode(1), :x),
            :e,
            Expr(:block, LineNumberNode(1), :y),
        ...
        @test parsestmt("try x catch e; y else z end", version = v"1.8") == Expr(
            :try,
            Expr(:block, LineNumberNode(1), :x),
            :e,
            Expr(:block, LineNumberNode(1), :y),
        ...
        @test parsestmt("try x catch e; y else z finally w end", version = v"1.8") == Expr(
            :try,
            Expr(:block, LineNumberNode(1), :x),
            :e,
            Expr(:block, LineNumberNode(1), :y),
        ...
        @test parsestmt("try x finally y catch e z end", ignore_warnings = true) == Expr(
            :try,
            Expr(:block, LineNumberNode(1), :x),
            :e,
            Expr(:block, LineNumberNode(1), :z),
        ...
        # empty recovery
        @test parsestmt("try x end", ignore_errors = true) == Expr(
            :try,
            Expr(:block, LineNumberNode(1), :x),
            false,
            false,
    ...
    @testset "Large integer macros" begin
        @test parsestmt("0x00000000000000001") == Expr(
            :macrocall,
            GlobalRef(Core, Symbol("@uint128_str")),
            nothing,
            "0x00000000000000001",
        )
    ...
    @testset "global/const/local" begin
        ...
        @test parsestmt("global const x = 1") ==
              Expr(:const, Expr(:global, Expr(:(=), :x, 1)))
        @test parsestmt("local const x = 1") ==
              Expr(:const, Expr(:local, Expr(:(=), :x, 1)))
        @test parsestmt("const global x = 1") ==
              Expr(:const, Expr(:global, Expr(:(=), :x, 1)))
        @test parsestmt("const local x = 1") ==
              Expr(:const, Expr(:local, Expr(:(=), :x, 1)))
        @test parsestmt("const x,y = 1,2") ==
              Expr(:const, Expr(:(=), Expr(:tuple, :x, :y), Expr(:tuple, 1, 2)))
    ...
    @testset "tuples" begin
        ...
        @test parsestmt("(; a=1; b=2)") == Expr(
            :tuple,
            Expr(:parameters, Expr(:parameters, Expr(:kw, :b, 2)), Expr(:kw, :a, 1)),
        )
        @test parsestmt("(a; b; c,d)") ==
    ...
    @testset "errors" begin
        ...
        @test parseall("a b", ignore_errors = true) ==
              Expr(:toplevel, LineNumberNode(1), :a, LineNumberNode(1), Expr(:error, :b))
        @test parsestmt("(x", ignore_errors = true) == Expr(:block, :x, Expr(:error))
        @test parsestmt("x do", ignore_errors = true) == Expr(:block, :x, Expr(:error, :do))
        @test parsestmt("x var\"y\"", ignore_errors = true) ==
              Expr(:block, :x, Expr(:error, :var, ErrorVal(), "y", ErrorVal()))
        @test parsestmt("var\"y", ignore_errors = true) == Expr(:var, :y, Expr(:error))
    end
