@testset "parser API" begin
    ...
    @testset "error/warning handling" begin
        parseshow(s; kws...) =
            sprint(show, MIME("text/x.sexpression"), parsestmt(SyntaxNode, s; kws...))
        ...
        # errors in literals
        @test parseshow("\"\\z\"", ignore_errors = true) ==
              "(string (ErrorInvalidEscapeSequence))"
        @test parseshow("'\\z'", ignore_errors = true) ==
              "(char (ErrorInvalidEscapeSequence))"
...
@testset "tokenize() API" begin
    ...
    @test untokenize.(tokenize(buf), Ref(buf)) ==
          [Vector{UInt8}("a"), Vector{UInt8}("-"), Vector{UInt8}("β")]

    @test kind(JuliaSyntax.Token()) == K"None"

    @test tokensplit("'\\") ==
          [K"'" => "'", K"ErrorInvalidEscapeSequence" => "\\", K"error" => ""]
end
