using JuliaSyntax:
    parse_int_literal, parse_uint_literal, parse_float_literal, unescape_julia_string
...
@testset "Integer parsing" begin
    ...
    @testset "HexInt numeric limits for different types" begin
        ...
        @test hexint("0xffffffffffffffffffffffffffffffff") ===
              UInt128(0xffffffffffffffffffffffffffffffff)
        @test (
            n = hexint("0x100000000000000000000000000000000");
    ...
    @testset "HexInt string length limits for different types" begin
        ...
        @test (n = hexint("0x000000000000000000000000000000000"); n isa BigInt && n == 0)
    end
    ...
    @testset "OctInt numeric limits for different types" begin
        ...
        @test octint("0o3777777777777777777777777777777777777777777") ===
              UInt128(0xffffffffffffffffffffffffffffffff)
        @test (
            n = octint("0o4000000000000000000000000000000000000000000");
            n isa BigInt && n == big"0x100000000000000000000000000000000"
        )
    end
    @testset "OctInt string length limits for different types" begin
        ...
        @test (
            n = octint("0o00000000000000000000000000000000000000000000");
            n isa BigInt && n == 0
        )
    end
...
function unesc_raw(str, is_cmd)
    io = IOBuffer()
    JuliaSyntax.unescape_raw_string(
        io,
        Vector{UInt8}(str),
        firstindex(str),
        lastindex(str)+1,
