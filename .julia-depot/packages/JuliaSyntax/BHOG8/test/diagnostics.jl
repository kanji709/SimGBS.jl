...
@testset "token errors" begin
    @test diagnostic("a\xf5b") ==
          Diagnostic(2, 2, :error, "invalid UTF-8 sequence \"\\xf5\"")
    @test diagnostic("# a\xf5b") ==
          Diagnostic(1, 5, :error, "invalid UTF-8 sequence \"# a\\xf5b\"")
    for c in
        ['\u00ad', '\u200b', '\u200c', '\u200d', '\u200e', '\u200f', '\u2060', '\u2061']
    ...
    @test diagnostic("\"X \u202a X\"") == Diagnostic(
        2,
        8,
        :error,
        "unbalanced bidirectional unicode formatting \"X \\u202a X\"",
    )
    @test diagnostic("#= \u202a =#") == Diagnostic(
        1,
        9,
        :error,
        "unbalanced bidirectional unicode formatting \"#= \\u202a =#\"",
    )
    @test diagnostic("\"X \u202a \$xx\u202c\"", allow_multiple = true) == [
        Diagnostic(
            ...
            7,
            :error,
            "unbalanced bidirectional unicode formatting \"X \\u202a \"",
        )
        Diagnostic(
            11,
            13,
    ...
    @test diagnostic("0x0.1") ==
          Diagnostic(1, 5, :error, "hex float literal must contain `p` or `P`")
end

@testset "parser errors" begin
    @test diagnostic("+ #==# (a,b)") == Diagnostic(
        2,
        7,
        :error,
        "whitespace not allowed between prefix function call and argument list",
    )
    @test diagnostic("1 -+ (a=1, b=2)") == Diagnostic(
        5,
        5,
        :error,
        "whitespace not allowed between prefix function call and argument list",
    ...
    # TODO: better range
    @test diagnostic("@A.\$x a") == Diagnostic(6, 5, :error, "invalid macro name")

    @test diagnostic("a, , b") == Diagnostic(4, 4, :error, "unexpected `,`")
    ...
    @test diagnostic("f(x, y #=hi=#\ng(z)") ==
          Diagnostic(7, 6, :error, "Expected `)` or `,`")
    @test diagnostic("(x, y \nz") == Diagnostic(6, 5, :error, "Expected `)` or `,`")
    @test diagnostic("function f(x, y \nz end") ==
          Diagnostic(16, 15, :error, "Expected `)` or `,`")

    @test diagnostic("sin. (1)") ==
          Diagnostic(5, 5, :error, "whitespace is not allowed here")
    @test diagnostic("x [i]") == Diagnostic(2, 2, :error, "whitespace is not allowed here")
    @test diagnostic("\nf() [i]") ==
          Diagnostic(5, 5, :error, "whitespace is not allowed here")
    @test diagnostic("\nf() (i)") ==
          Diagnostic(5, 5, :error, "whitespace is not allowed here")
    ...
    @test diagnostic("\nf() {i}") ==
          Diagnostic(5, 5, :error, "whitespace is not allowed here")
    @test diagnostic("\n@ m") == Diagnostic(3, 3, :error, "whitespace is not allowed here")
    @test diagnostic("\nusing a .b") ==
    ...
    @test diagnostic("(for i=1; println())") == Diagnostic(20, 19, :error, "Expected `end`")
    ...
    @test diagnostic("", rule = :atom) == Diagnostic(1, 0, :error, "premature end of input")
end
...
@testset "parser warnings" begin
    ...
    @test diagnostic("export :x") == Diagnostic(8, 9, :error, "expected identifier")
...
@testset "diagnostics for literal parsing" begin
    ...
    # Char
    @test diagnostic("x = ''") == Diagnostic(6, 5, :error, "empty character literal")
    ...
    @test diagnostic("x = '\\k'") == Diagnostic(6, 7, :error, "invalid escape sequence")
    @test diagnostic("'\\", allow_multiple = true) == [
