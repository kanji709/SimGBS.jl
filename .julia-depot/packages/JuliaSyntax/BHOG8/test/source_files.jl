...
@testset "SourceFile position indexing" begin
    ...
    if Base.VERSION >= v"1.4"
        ...
        @test eval(Meta.parse("SourceFile(\"abcd\", first_index=10)[begin+1:end-1]")) ==
              "bc"
    end
end
...
@testset "highlight()" begin
    ...
    # context lines
    @test sprint(
        io->highlight(io, src, 8:13; context_lines_before = 0, context_lines_after = 0),
    ) == """
αβγδ
#└─┘"""
...
+-*/"""
    ...
    @test sprint(
        io->highlight(io, src, 1:18, context_lines_inner = 0),
        context = :color=>true,
    ) ==
